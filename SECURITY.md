...
- **No rate limiting anywhere.** Nothing throttles logins, registrations or writes, and
  ...
  in front of it. By default, new invite codes are 16 hex characters (64 random bits). Instances
  with `INVITE_PREFIX` use a readable sequence plus a 10-character random suffix (50 random bits),
  for example `FORTA-002-K7MX9Q2T4P`. Older and manually seeded codes still work because
  validation compares exact strings rather than enforcing a format. The only hard limit
